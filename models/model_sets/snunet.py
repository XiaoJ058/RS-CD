...
class SNUNet(nn.Module, KaimingInitMixin):
    def __init__(self, in_ch=3, out_ch=1, width=16):
        super().__init__()
    ...
    def forward(self, t1, t2):
        ...
        change = self.ca_inter(out) * (out + m_intra.repeat(1,4,1,1))
        change = self.conv_out(change)

        return F.upsample(change, t1_size[2:], mode='bilinear')
